...
const pagePath = path.join(__dirname, 'src', 'pages', 'admin', 'AdminB2BOrdersPage.jsx');
const featurePath = path.join(__dirname, 'src', 'features', 'b2b', 'components', 'Admin', 'AdminB2BOrders.jsx');

const adminOrdersCode = `import React, { useState, useEffect, useRef } from "react";
  ...
  return (
    <div className="flex gap-6 h-[800px]">
      ...
      <div className="w-1/3 bg-white border border-gray-200 rounded-xl overflow-hidden flex flex-col shadow-sm">
        <div className="p-4 border-b bg-gray-50 font-bold text-gray-800">Danh sách B2B Orders</div>
        <div className="flex-1 overflow-y-auto">
          {orders.map(o => (
            <div 
              key={o._id} 
              onClick={() => setSelectedOrder(o)}
              className={\`p-4 border-b cursor-pointer transition-colors \${selectedOrder?._id === o._id ? 'bg-blue-50 border-blue-200' : 'hover:bg-gray-50'}\`}
            >
              <h4 className="font-bold text-gray-800 truncate">{o.companyName}</h4>
              <p className="text-sm text-gray-500 truncate">{o.productOrService?.name}</p>
        ...
        {selectedOrder ? (
          <>
            <div className="p-6 border-b flex justify-between items-start">
              <div>
                <h2 className="text-xl font-bold text-gray-800">{selectedOrder.companyName}</h2>
                <p className="text-gray-500 text-sm">{selectedOrder.productOrService?.name} (x{selectedOrder.quantity})</p>
              ...
              <div className="flex flex-col items-end gap-2">
                <select 
                  ...
                  className="px-3 py-2 border rounded-lg text-sm bg-gray-50 outline-none focus:border-blue-500"
                >
                ...
                {selectedOrder.quotePdfUrl && (
                  <a href={selectedOrder.quotePdfUrl} target="_blank" rel="noreferrer" className="text-blue-600 text-sm hover:underline mt-2 flex items-center gap-1">
                    Xem PDF Báo giá
                  </a>
                )}
              </div>
                ...
                return (
                  <div key={i} className={\`flex gap-3 \${isMe ? 'flex-row-reverse' : ''}\`}>
                    ...
                    <div className={\`max-w-[75%] \${isMe ? 'items-end' : 'items-start'} flex flex-col\`}>
                      ...
                      <div className={\`px-4 py-2 rounded-2xl \${isMe ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white border text-gray-800 rounded-tl-none'} shadow-sm text-sm\`}>
                        {c.text}
                      </div>
                    </div>
                  </div>
            ...
            <form onSubmit={handleSendComment} className="p-4 border-t bg-white flex gap-2">
              <input
                ...
                className="flex-1 border border-gray-300 rounded-xl px-4 py-2.5 outline-none focus:border-blue-500 text-sm"
              />
              <button
                type="submit"
...
const adminPageCode = `import React from "react";
import AdminB2BOrders from "../../features/b2b/components/Admin/AdminB2BOrders";

const AdminB2BOrdersPage = () => {
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Quản lý Hợp đồng B2B</h1>
      <AdminB2BOrders />
    </div>
  );
};

export default AdminB2BOrdersPage;
`;

fs.writeFileSync(featurePath, adminOrdersCode);
fs.writeFileSync(pagePath, adminPageCode);
console.log("Created Admin B2B Order components.");
