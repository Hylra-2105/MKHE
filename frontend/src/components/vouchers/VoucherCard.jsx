...
const VoucherCard = ({ 
  ...
  const getDiscountText = () => {
    if (voucher.type === "PERCENTAGE") {
      return t("voucher.discount_percentage", { value: voucher.discountValue, defaultValue: `Giảm ${voucher.discountValue}%` });
    }
    if (voucher.type === "FIXED_AMOUNT") {
      return t("voucher.discount_fixed", { value: formatNumber(voucher.discountValue), defaultValue: `Giảm ${formatNumber(voucher.discountValue)}đ` });
    }
    if (voucher.type === "FREE_SHIP") {
      return t("voucher.free_shipping", { defaultValue: "Miễn phí vận chuyển" });
    }
    return "";
  };
  ...
  const getMinOrderText = () => {
    if (voucher.minOrderValue > 0) {
      return t("voucher.min_order", { value: formatNumber(voucher.minOrderValue), defaultValue: `Đơn tối thiểu ${formatNumber(voucher.minOrderValue)}đ` });
    }
    return t("voucher.no_min_order", { defaultValue: "Không giới hạn đơn tối thiểu" });
  };
  ...
  return (
    <div className={`relative flex w-full bg-mkhe-card rounded-xl overflow-hidden border-2 transition-all ${
      ...
      {/* Nét đứt ngăn cách (Ticket Shape) */}
      ...
      <div className="flex-1 p-4 flex flex-col justify-between">
        <div>
          ...
          {voucher.type === "PERCENTAGE" && voucher.maxDiscount > 0 && (
            <p className="text-xs text-mkhe-text/60">{t("voucher.max_discount", { value: formatNumber(voucher.maxDiscount), defaultValue: `Tối đa ${formatNumber(voucher.maxDiscount)}đ` })}</p>
          )}
        </div>
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          {mode === "selector" ? (
            ...
            >
              {isSelected ? t("voucher.applying", { defaultValue: "Đang áp dụng" }) : t("voucher.use_now", { defaultValue: "Dùng ngay" })}
            </button>
          ) : (
            // Wallet Mode
            <div className="flex gap-2">
              {voucher.isO2O && onShowQR && (
                <button
                  ...
                  title={t("voucher.scan_at_counter", { defaultValue: "Mã quét tại quầy" })}
                >
              ...
              >
                {t("voucher.use_online", { defaultValue: "Dùng Online" })}
              </button>
            </div>
          )}
        </div>
